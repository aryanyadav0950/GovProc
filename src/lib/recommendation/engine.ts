...
export function generateProcurementRecommendation(
  ...
  if (kpiCount > 0) {
    for (const kpi of pilot.kpis) {
      ...
      // Some KPIs are better when the value is LOWER,
      // such as latency, response time, processing time,
      // completion time, error rate, failure rate, and downtime.
      const lowerIsBetter =
        /latency|response time|processing time|completion time|error rate|failure rate|downtime/i.test(
          kpi.name
        );

      let pct = 70;

      if (kpi.target > 0 && latestVal > 0) {
        pct = lowerIsBetter
          ? (kpi.target / latestVal) * 100
          : (latestVal / kpi.target) * 100;
      }
